...
import com.cookiegames.smartcookie.databinding.ActivityMainBtmBinding
...
abstract class BrowserActivity : ThemableBrowserActivity(), BrowserView, UIController,
  ...
  private var btmBinding: ActivityMainBtmBinding? = null
  private lateinit var toolbarBinding: ToolbarContentBinding
  private lateinit var dialogAppLockBinding: DialogAppLockBinding

  private val toolbar: Toolbar
    get() = binding?.toolbarLayout?.toolbar ?: btmBinding!!.toolbarLayout.toolbar
  private val progress_view: AnimatedProgressBar
    get() = binding?.toolbarLayout?.progressView ?: btmBinding!!.toolbarLayout.progressView
  private val left_drawer: FrameLayout
    get() = binding?.leftDrawer ?: btmBinding!!.leftDrawer
  private val right_drawer: FrameLayout
    get() = binding?.rightDrawer ?: btmBinding!!.rightDrawer
  private val content_frame: FrameLayout
    get() = binding?.browserContentLayout?.contentFrame
      ?: btmBinding!!.browserContentLayout.contentFrame
  val drawer_layout: DrawerLayout
    get() {
      return binding?.drawerLayout ?: btmBinding!!.drawerLayout
    }
  private val toolbar_layout: LinearLayout
    get() = binding?.toolbarLayout?.toolbarLayout ?: btmBinding!!.toolbarLayout.toolbarLayout
  private val ui_layout: LinearLayout
    get() = binding?.uiLayout ?: btmBinding!!.uiLayout
  private val tabs_toolbar_container: FrameLayout
    get() = binding?.toolbarLayout?.tabsToolbarContainer
      ?: btmBinding!!.toolbarLayout.tabsToolbarContainer
  private val search_ssl_status: ImageView
    get() = toolbarBinding.searchLayout.searchSslStatus
  private val search_refresh: ImageView
    get() = toolbarBinding.searchLayout.searchRefresh
  private val search_bar: RelativeLayout
    get() = binding?.searchInterfaceLayout?.searchBar
      ?: btmBinding!!.searchInterfaceLayout.searchBar
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    if (userPreferences.bottomBar) {
      btmBinding = ActivityMainBtmBinding.inflate(layoutInflater)
  ...
  private fun initialize(savedInstanceState: Bundle?) {
    ...
    isDarkTheme =
      userPreferences.useTheme == AppTheme.DARK || userPreferences.useTheme == AppTheme.BLACK || isIncognito()
    ...
    tabsView = if (shouldShowTabsInDrawer) {
      TabsDrawerView(this, userPreferences = userPreferences).also(
        findViewById<FrameLayout>(
          getTabsContainerId()
        )::addView
      )
    } else {
      TabsDesktopView(this, userPreferences = userPreferences).also(
        findViewById<FrameLayout>(
          getTabsContainerId()
        )::addView
      )
    ...
    bookmarksView = BookmarksDrawerView(
      this,
      this,
      userPreferences = userPreferences
    ).also(findViewById<FrameLayout>(getBookmarksContainerId())::addView)
...
//      background.tint(getSearchBarColor(primaryColor, primaryColor))
    ...
    if (userPreferences.passwordChoiceLock == PasswordChoice.CUSTOM) {
      ...
      dialogAppLockBinding = DialogAppLockBinding.inflate(layoutInflater)
      val customLayout: View = dialogAppLockBinding.root
      builder.setView(customLayout)
      builder.setCancelable(false)

      builder.setPositiveButton(resources.getString(R.string.action_ok)) { dialog, _ ->
        if (dialogAppLockBinding.textFieldText.text.toString() == userPreferences.passwordTextLock) {
          dialog.cancel()
        } else {
          this.finishAffinity()
        }
  ...
  override fun showCloseDialog(position: Int) {
    ...
    BrowserDialog.showWithIcons(
      this, getString(R.string.dialog_title_close_browser),
      ...
      DialogItem(
        ...
        icon = drawable(R.drawable.ic_delete_all),
        onClick = this::closeBrowser
      ),
      DialogItem(
        title = R.string.close_app,
        icon = drawable(R.drawable.ic_action_delete),
  ...
  override fun handleNewTab(
    newTabType: LightningDialogBuilder.NewTab,
    url: String,
    addToIndex: Boolean
  ) {
      ...
      LightningDialogBuilder.NewTab.BACKGROUND -> {
        ...
        val snackbar = Snackbar
          .make(
            ...
            resources.getString(R.string.new_tab_opened),
  ...
  private fun setIsLoading(isLoading: Boolean) {
    if (searchView?.hasFocus() == false) {
      search_ssl_status.updateVisibilityForContent()
      search_refresh.setImageResource(
        if (isLoading) R.drawable.ic_action_delete else
          if (isDarkTheme) {
    ...
    when (v.id) {
      ...
      R.id.download_button -> {
        ...
        sendIntent.setClassName(
          "com.cookiejarapps.smartcookieweb_ytdl",
      ...
      R.id.button_search -> {
        ...
        findResult =
          presenter?.findInPage(findViewById<EditText>(R.id.search_query).text.toString())
      }
    }
  }
